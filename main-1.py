...
def calculate_time(some_list, some_name):
    ...
    if some_route[len(some_route) - 1] == some_name and not flag:
        ...
        # if first_stop_time_forward - current_time <= 10:
        #     dictionary[route_name, some_route[station_number + 1]] = first_stop_time_forward - current_time
        #     time_range = True
        # if first_stop_time_backwards - current_time <= 10:
...
while True:
    ...
    for j in range(number_of_routes):
        if find_station_name(list_of_routes[j], answer):
            checked = True
            if not calculate_time(list_of_routes[j], answer):
                timing = False
    ...
    if checked:
        ...
        for w in sorted(dictionary, key=dictionary.get, reverse=False):
            print(f'{w[0]},', "destination", f'{w[1]},', dictionary[w], "min")

    if not checked:
        print("No such station!")
    # elif not timing:
    #     print("Ooops, no buses within 10 minutes!")
